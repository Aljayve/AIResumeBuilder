    ...
    <div className="relative">
      ...
      >
        <span className="mr-3 overflow-hidden rounded-full h-11 w-11">
          {hasAvatar ? (
            <img src={user?.avatar?.url ?? ""} alt={name} className="w-full h-full object-cover" />
          ) : (
            <span className="flex items-center justify-center w-full h-full text-sm font-semibold text-white bg-brand-500">
              {initials}
            </span>
          )}
        </span>
        <span className="hidden mr-1 font-medium text-theme-sm md:block">{name}</span>
        <ChevronDown
          size={16}
          className={`hidden text-gray-500 dark:text-gray-400 transition-transform duration-200 md:block ${isOpen ? "rotate-180" : ""}`}
        />
      </button>
