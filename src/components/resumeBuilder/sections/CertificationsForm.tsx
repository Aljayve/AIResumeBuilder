...
import type { Certification } from "../../../store/resumeBuilderStore";
...
export default function CertificationsForm() {
    ...
    const setItems = (next: Certification[]) => updateResume({ certifications: next });

    const add = () => {
        const id = `${Date.now()}-${Math.random()}`;
        const next: Certification[] = [...items, { id, name: "", issuer: "", issueDate: "" }];
        setItems(next);
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
            <div className="mb-5 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="font-semibold text-gray-800 dark:text-white/90">Certifications</h3>
                </div>
            ...
            <div className="space-y-3">
                {items.length === 0 && (
                    <p className="text-sm italic text-gray-400">No certifications added yet.</p>
                )}
                {items.map((item, i) => (
                    <MoveEditSection
                        key={item.id}
                    ...
                    >
                        <div className="flex flex-col gap-4">
                            <Input label="Certification name" value={item.name} onChange={(e) => update(i, "name", e.target.value)} required />
                            <Input label="Issuer" value={item.issuer} onChange={(e) => update(i, "issuer", e.target.value)} />
                            <Input label="Issue date" value={item.issueDate} onChange={(e) => update(i, "issueDate", e.target.value)} />
                        </div>
                    </MoveEditSection>
                ))}
            </div>
